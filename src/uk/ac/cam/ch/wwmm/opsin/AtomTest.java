...
public class AtomTest {
	...
	public void testGetIncomingValency() throws StructureBuildingException {
		...
		assertEquals("No bonds", 0, 
				ssBuilder.build("C", idm).getAtomList().get(0).getIncomingValency());
		assertEquals("One bond", 1, 
				ssBuilder.build("CC", idm).getAtomList().get(0).getIncomingValency());
		assertEquals("Two bonds", 2, 
				ssBuilder.build("C(C)C", idm).getAtomList().get(0).getIncomingValency());
		assertEquals("Double bond", 2, 
				ssBuilder.build("C=O", idm).getAtomList().get(0).getIncomingValency());
		assertEquals("Triple bond", 3, 
				ssBuilder.build("C#C", idm).getAtomList().get(0).getIncomingValency());
		assertEquals("One bond", 1, 
				ssBuilder.build("CC=CC#N", idm).getAtomList().get(0).getIncomingValency());
	}
