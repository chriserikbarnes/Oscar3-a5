...
class ValencyChecker {
	...
	private static ValencyChecker myInstance = null;
	private Map<String, Integer> expectedDefaultValency;//used to decide on the likely valency state
	private Map<String, Integer> valencyInHW;//used to decide whether an atom has spare valency in a ring, these are the same as specified in the Hantzch-Widman system
	private Map<String, HashMap<Integer, Integer[]>> possibleStableValencies;//used to decide on the likely valency state

	private ValencyChecker() {
		...
		possibleStableValencies.get("B").put(-1, new Integer[]{3});
		...
		possibleStableValencies.get("S").put(-1, new Integer[]{2});
	...
	private static ValencyChecker getInstance() {
		if(myInstance == null) myInstance = new ValencyChecker();
		return myInstance;
	}

	private Integer getMaxValency(String symbol, int charge) {
	...
	static boolean checkValency(Atom a) {
		...
		if (a.getValency()!=null){
			...
			maxVal=vc.getMaxValency(symbol, charge);
			if(maxVal==null) return true;
		}
		return valency <= maxVal;
	}

	/** Check whether enough spare valency exists on an atom
	 ...
	 * spareValency is taken into account
	 * @param atom you are interested in
	...
	static boolean checkValencyAvailableForReplacementByHeteroatom(Atom a, String heteroatom) {
		...
		Integer maxValOfHeteroAtom =vc.getMaxValency(heteroatom, 0);
	...
	static Integer getDefaultValency(String element, int charge) {
		...
		ValencyChecker vc =getInstance();
		return vc.expectedDefaultValency.get(element);
	}
	...
	static Integer getHWValency(String element) {
		ValencyChecker vc =getInstance();
		return vc.valencyInHW.get(element);
	}
	...
	static Integer getMaximumValency(String element, int charge) {
		ValencyChecker vc =getInstance();
		return vc.getMaxValency(element, charge);
	}
	...
	static Integer[] getPossibleValencies(String element, int charge) {
		ValencyChecker vc =getInstance();
		if (vc.possibleStableValencies.get(element)==null){return null;}
		return vc.possibleStableValencies.get(element).get(charge);
	}
}
