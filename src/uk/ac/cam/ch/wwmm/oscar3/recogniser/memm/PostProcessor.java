...
final class PostProcessor {
	...
	private static Pattern rnPattern = Pattern.compile(".*(tions?|ing|ed|ates?|ativ(e|e?ly)|" +
	...
	public static int filterEntity(String surf, String type) {
		...
		if(!surf.matches(".*[a-zA-Z].*") && !type.equals("CPR")) {
			...
			return 3;
		} else if(type.equals("RN") && !(rnPattern.matcher(surf).matches() || !surf.matches(".*[a-zA-Z].*"))) {
			return 4;
		} else if(type.equals("CPR") && !surf.matches(".+[" + StringTools.hyphens + "]")) {
			return 5;
		} else if((type.equals("CPR") || type.equals("CJ") || type.equals("ASE"))
				&& surf.matches(".+ .+")) {
			return 6;
			...
			return 8;
		} else if(surf.matches("(\\.|\\,|:|\\d+(\\.\\d+)?|=|at|is|has|with|are|on|of|and|or|were|in|as|was)\\s+.*") || 
				surf.matches(".*\\s+(\\.|\\,|:|=|at|is|has|with|are|on|of|and|or|were|in|as|was)")) {
			return 9;
		} else if(TokenTypes.oxidationStatePattern.matcher(surf).matches()) {
			return 10;
		} else if(!StringTools.bracketsAreBalanced(surf) && surf.matches(".*\\s.*")) {
			return 11;
			//Fix things for alternate annotation scheme
		} else if((type.length() < 4) && (!noPC && ExtractTrainingData.getInstance().nonChemicalWords.contains(surf))) {
			return 12;
		} else if(ChemNameDictSingleton.hasStopWord(surf) || TermSets.getStopWords().contains(surf)) {
			return 13;
		}
		return 0;
	}
	...
	public void filterEntities() {
		...
		for(NamedEntity ne : neList) {
			if(filterEntity(ne) > 0) entities.remove(ne);
		}
	}
	...
	public Set<NamedEntity> getBlocked() {
		...
		for(NamedEntity ne : getSorted()) {
			...
			for(int i=ne.getTokens().get(0).getId();i<=ne.getTokens().get(ne.getTokens().size()-1).getId();i++) {
	...
	public void removeBlocked() {
		for(NamedEntity ne : getBlocked()) {
			if(Oscar3Props.getInstance().verbose) System.out.println("Removing: " + ne);
			entities.remove(ne);
		}
	}
	...
	private List<NamedEntity> getSorted() {
		...
		Collections.sort(sorted, Collections.reverseOrder(new Comparator<NamedEntity>() {
