...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<script type="text/javascript"
					src="/art_tool_web/javascript/prototype-1.6.0.2.js" />
				<script type="text/javascript"
					src="/art_tool_web/javascript/scriptaculous.js" />
				<script type="text/javascript"
					src="/art_tool_web/javascript/art_tool.js" />

				<xsl:if test="/PAPER/METADATA/FILENO">
					<meta>
						<xsl:attribute name="name">fileno</xsl:attribute>
						<xsl:attribute name="content"><xsl:value-of
								select="/PAPER/METADATA/FILENO" />
						</xsl:attribute>
					</meta>
				</xsl:if>
				<style type="text/css">
					...
					div.Conclusion, div.Results,
					div.Goal,div.Method,div.Object,div.Experiment,div.Observation,div.Hypothesis,div.Motivation,div.Problem,div.Background,div.Model,div.Example{
					display:inline; padding-left: 2px; padding-right:
					2px; padding-top: 1px; padding-bottom:1px;
					font-weight: bold; color:white; } div.Background{
					background-color: #666; } div.Conclusion {
					background-color: #600; } div.Example{
					background-color: #660; } div.Experiment{
					background-color: #060; } div.Goal {
					background-color: #066; } div.Hypothesis{
					background-color: #006; } div.Method {
					background-color: #606; } div.Model{
					background-color: #c00; } div.Motivation{
					background-color: #cc0; } div.Object {
					background-color: #0c0; } div.Observation{
					background-color: #0cc; } div.Problem{
					background-color: #00c; } div.Results {
					background-color: #c0c; }

					.sidebartitle { font-style: italic; }

					div.sidebar { text-align: center; right: 0px; width:
					...
					<xsl:if test="$viewer">
						div.mainpage { height: 99%; width: 70%;
						position: absolute; top: 2px; left: 0px; bottom:
						2px; padding: 2px; right: 320px; overflow: auto;
						}
					</xsl:if>
					...
					span.CM { background-color: #FFFF33; }

					span.ONT { background-color: #FFAA00; }

					span.ASE { background-color: #7700FF; }

					span.CJ { background-color: #AAAAFF; }

					span.RN { background-color: #00FF00; }

					span.CPR { background-color: #FF00FF; }

					span.DATA { background-color: #FF7777; }

					span.CUST { background-color: #CCCCCC; }

					<!-- Used by ScrapBook -->

					p.snippet { border-style: dotted; border-width: 1px;
					}
	  ...
	  function clearImage() {
	    if(getViewer()) {
	    	...
	    	document.getElementById("molImg").setAttribute("src", "/ViewMol");
	    }
	  }
	  ...
	  function view(elemID) {
	    ...
	    if(getViewer()) {
		    ...
		    if(x["InChI"] != null && x["SMILES"] != null) {
				document.getElementById("molImg").setAttribute("src", "/ViewMol?inchi=" + encodeURIComponent(x["InChI"]) + "&smiles=" + encodeURIComponent(x["SMILES"]));		    
			} else if(x["InChI"] != null || x["SMILES"] != null) {
				document.getElementById("molImg").setAttribute("src", "/ViewMol?inchi=" + encodeURIComponent(x["InChI"]));
			} else if(x["SMILES"] != null) {
				document.getElementById("molImg").setAttribute("src", "/ViewMol?smiles=" + encodeURIComponent(x["SMILES"]));
			} else if(x["Element"] != null) {
				document.getElementById("molImg").setAttribute("src", "/ViewMol?element=" + encodeURIComponent(x["Element"]));
		    } else {
				document.getElementById("molImg").setAttribute("src", "/ViewMol");
		    }
	  ...
	  function clickon(elemID) {
		...
		document.location = "/NEViewer?" + namepart + typepart + smilespart + inchipart + ontpart;
	  }
				...
				<xsl:choose>
					<xsl:when test="/PAPER/scrapbook/relitems">
						<div class="sidebar2">
							<ul>
								<xsl:for-each
									select="/PAPER/scrapbook/relitems/rel">
									<li onclick="addRel('{.}')">
										<xsl:value-of select="." />
									</li>
								</xsl:for-each>
		...
		<form name="appletForm">
		  <applet code="uk.ac.cam.ch.wwmm.viewerapplet.ViewerApplet"
		    ...
		    <param name="scriptable" value="true" />
							...
							<div id="oscarkey" style="display:none">
					...
					<xsl:for-each select="PAPER">
						<xsl:choose>
							<xsl:when
								...
								<form action="ScrapBook"
									method="POST">
									<xsl:if
										test="//scrapbook[@mode='selectorEditor']">
										<input type="hidden"
											name="action" value="selecteditsubmit" />
										<input type="hidden"
											name="type">
											<xsl:attribute
												name="value"><xsl:value-of
													select="//scrapbook/@selType" />
											</xsl:attribute>
										</input>
									</xsl:if>
									<xsl:if
										test="//scrapbook[@mode='textFieldEditor']">
										<input type="hidden"
											name="action" value="selecteditsubmit" />
										<input type="hidden"
											name="type">
											<xsl:attribute
												name="value"><xsl:value-of
													select="//scrapbook/@txtType" />
											</xsl:attribute>
										</input>
									</xsl:if>

									<xsl:if
										test="//scrapbook[@mode='booleanEditor']">
										<input type="hidden"
											name="action" value="booleaneditsubmit" />
										<input type="hidden"
											name="attrName">
											<xsl:attribute
												name="value"><xsl:value-of
													select="//scrapbook/@attrName" />
											</xsl:attribute>
										</input>
									</xsl:if>
									<input type="hidden" name="name">
										<xsl:attribute name="value"><xsl:value-of
												select="//scrapbook/@name" />
										</xsl:attribute>
									</input>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<form method="POST" action="ART">
					...
					<xsl:if
						test="/PAPER/scrapbook[@mode='editor' or @mode='attreditor']">
	...
	<xsl:template name="scrapbook" match="scrapbook">
		<xsl:if test="not($printable)">
			<p>
				<xsl:element name="a">
					<xsl:attribute name="href">ART?action=show&amp;name=<xsl:value-of
							select="@name" />
					</xsl:attribute>
					Show
				</xsl:element>
				...
				<xsl:element name="a">
					...
					<!-- <xsl:attribute name="onclick">addOscarKey();return false;</xsl:attribute> -->
					Auto Annotate
				...
				<xsl:element name="a">
					...
					Clear Oscar Annotations
				</xsl:element>
				|
				<xsl:element name="a">
					<xsl:attribute name="href">ART?action=clearART&amp;name=<xsl:value-of
							select="@name" />
					</xsl:attribute>
					Clear ART Annotations
				</xsl:element>
				|
				<xsl:element name="a">
	      ...
	      wD.getElementById('thisForm').submit();		
			...
			<p>
				<xsl:if
					test="self::node()[@mode='editor' or @mode='attreditor']">
					<script>
						<xsl:if test="self::node()[@mode='editor']">
							function addNeAtSel(ne) {
						</xsl:if>
						<xsl:if
							test="self::node()[@mode='attreditor']">
							function moveNeToSel(neid) {
						</xsl:if>
						if(document.selection) { r =
						document.selection.createRange(); re =
						/ci="\d+"/g; nre = /\d+/; ht = r.htmlText;
						matches = ht.match(re); start =
						parseInt(matches[0].match(nre)) end =
						parseInt(matches[matches.length-1].match(nre)) +
						1 n = r.parentElement(); } else { r =
						window.getSelection().getRangeAt(0); start =
						r.startContainer.parentNode.getAttribute("ci");
						start = parseInt(start); if(r.startOffset == 1)
						start += 1; end =
						r.endContainer.parentNode.getAttribute("ci");
						end = parseInt(end); if(r.endOffset == 1) end +=
						1; n = r.startContainer; } while(n.tagName !=
						'FORM' &amp;&amp; n.tagName != 'HTML') { n =
						n.parentNode; } if(n.tagName == 'HTML' ||
						!n.getAttribute('name')) { alert("not in a
						form!"); } sid = n.getAttribute('name');
						if(!sid) { alert("bad place for an annotation");
						} else { cmd = "ART?sid=" + sid + "&amp;start="
						+ start + "&amp;end=" + end;
						<xsl:if test="self::node()[@mode='editor']">
