...
public final class NamedEntity extends ResolvableStandoff {
	...
	/**Compares the confidence score (or a suitable proxy) of the named entity 
	 * with that of another named entity, for use when deciding which of two
	 * overlapping named entities to discard.
	 * 
	 */
	@Override
	public int compareConfidenceTo(ResolvableStandoff other) {
	...
	/**Compares the type of the named entity 
	 * with that of another named entity, for use when deciding which of two
	 * overlapping named entities to discard.
	 * 
	 */
	...
	public int compareTypeTo(ResolvableStandoff other) {
		if(other instanceof NamedEntity) {
			NamedEntity otherNe = (NamedEntity)other;
			// Deprioritise ONT if necessary
			if(type.equals(NETypes.ONTOLOGY) && !otherNe.type.equals(NETypes.ONTOLOGY) && !priorityOnt) {
				return -1;
			} else if(!type.equals(NETypes.ONTOLOGY) && otherNe.type.equals(NETypes.ONTOLOGY) && !priorityOnt) {
				return 1;
			} else {
				return 0;
			}
	 ...
	 */
	@Override
	public int compareTypeToIfSameString(ResolvableStandoff other) {
		if(other instanceof NamedEntity) {
			NamedEntity otherNe = (NamedEntity)other;
	...
	/**Sets whether this named entity, if an ontology term, should take priority
	 * over other named entities when working out which entity to discard when
	 * resolving overlaps.
	 * 
	 * @param priorityOnt
	 */
	public void setPriorityOnt(boolean priorityOnt) {
		this.priorityOnt = priorityOnt;
	}
