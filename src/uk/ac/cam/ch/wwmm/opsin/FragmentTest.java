...
public class FragmentTest {
	...
	public void testImportFrag() {
		...
		newFrag.addBond(new Bond(3, 4, 1));
	...
	public void testFindBond() {
		...
		frag.addBond(new Bond(2, 4, 2));
		frag.addBond(new Bond(1, 2, 1));
		frag.addBond(new Bond(1, 3, 3));
...
//		Fragment parentFrag = ssBuilder.build("CC", idManager);
//		frag = ssBuilder.build("N", idManager);
...
//		frag = ssBuilder.build("C", idManager);
...
//	}
	...
	public void testPickUpIndicatedHydrogen() throws StructureBuildingException {
		//TODO: this, properly.
		SSMILESFragmentBuilder ssBuilder = new SSMILESFragmentBuilder();
		Fragment pyrrole = ssBuilder.build("NC=CC=C", new IDManager());
	...
	public void testConvertSpareValenciesToDoubleBonds() throws Exception {
		SSMILESFragmentBuilder ssBuilder = new SSMILESFragmentBuilder();
		IDManager idManager = new IDManager();
		FragmentManager fm =new FragmentManager(ssBuilder, null, idManager);
		Fragment dhp = ssBuilder.build("C1=CCC=CC1", idManager);
		dhp.convertHighOrderBondsToSpareValencies();
		dhp.convertSpareValenciesToDoubleBonds(fm);
		for(Atom a : dhp.getAtomList()) {
			assertEquals("All atoms have no sv", 0, a.getSpareValency());
		}
		Fragment funnydiene = ssBuilder.build("C(=C)C=C", idManager);
		funnydiene.convertHighOrderBondsToSpareValencies();
		funnydiene.convertSpareValenciesToDoubleBonds(fm);
		for(Atom a : funnydiene.getAtomList()) {
			assertEquals("All atoms have no sv", 0, a.getSpareValency());
		}
		Fragment naphthalene = ssBuilder.build("C1=CC=CC2=CC=CC=C12", idManager);
		naphthalene.convertHighOrderBondsToSpareValencies();
		naphthalene.convertSpareValenciesToDoubleBonds(fm);
		for(Atom a : naphthalene.getAtomList()) {
			assertEquals("All atoms have no sv", 0, a.getSpareValency());
		}
		Fragment pentalene = ssBuilder.build("C12C(=CC=C1)C=CC=2", idManager);
		pentalene.convertHighOrderBondsToSpareValencies();
		pentalene.convertSpareValenciesToDoubleBonds(fm);
